		...
		<div class="container">
			<div class="header">
				<div class="button-group">
					<button class="new-upload" name="new-upload" :class="[selectedSection == SECTION?.NEW_UPLOAD ? 'active' : '']"
						@click="switchSectionHandler">
						{{ filesStore.translate('New Upload') }}
					</button>
					<button class="recent" name="recent" :class="[selectedSection == SECTION?.RECENT ? 'active' : '']"
						@click="switchSectionHandler">
			...
			<div class="body">
				<div :class="[this.selectedSection == SECTION.NEW_UPLOAD ? '' : 'hide']">
					<UploadFile @uploadFiles="uploadFiles" :uploadStatus="uploadStatus" :translate="filesStore.translate" />
				</div>
				<div :class="[this.selectedSection == SECTION.RECENT ? '' : 'hide']">
					<ListFiles :files="files" :loadStatus="loadStatus" :lastSyncedDate="lastSyncedDate" :viewAllUploads="true"
						:remove="true" :update="true" :translate="filesStore.translate"
						:filesRetrievalMode="filesStore.filesRetrievalMode" @getFiles="getFiles" @removeFile="removeFile"
